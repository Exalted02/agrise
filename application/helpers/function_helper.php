...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '')
{
    ...
    // print_r("address_id: " . $address_id . "\n");
    // print_r("delivery_charge: " . $delivery_charge . "\n");
    // print_r("total: " . $total . "\n");
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $order_type = '', $for_app = NULL, $draftFilter = 1)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        $t->db->select('oi.*,p.id as product_id,p.is_cancelable,p.cancelable_till,p.is_returnable,p.image,p.name,p.download_allowed,p.download_link,p.is_prices_inclusive_tax,
        ...
        if (isset($order_details[$i]['status']) && !empty($order_details[$i]['status'])) {
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            ...
            if (!empty($order_item_data)) {
                ...
                $order_item_data[$k]['image_sm'] = (!empty($order_item_data[$k]['image']) && file_exists(FCPATH . $order_item_data[$k]['image']))
                ...
                $order_item_data[$k]['image_md'] = (!empty($order_item_data[$k]['image']) && file_exists(FCPATH . $order_item_data[$k]['image']))
                ...
                // Full-size image
                $order_item_data[$k]['image'] = (!empty($order_item_data[$k]['image']) && file_exists(FCPATH . $order_item_data[$k]['image']))
        ...
        if ($wallet_refund == true) {
            if ($payment_method != 'cod') {
                ...
                if ($is_delivery_charge_returnable == 1) {
                    $returnable_amount = $order_details[0]['total'] + $order_details[0]['delivery_charge'] + $order_details[0]['total_tax_amount'] - $order_details[0]['promo_discount'];
                } else {
                    $returnable_amount = $order_details[0]['total_payable'] - $order_details[0]['delivery_charge'];
                }
