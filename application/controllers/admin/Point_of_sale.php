    ...
    public function calculate_cart_total()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!empty($cart_data)) {
                ...
                if (!empty($cart_data)) {
                    $product_variant_ids = array_column($cart_data, 'variant_id');
                    $quantities = array_column($cart_data, 'quantity');
                    ...
                    $bulk_discount = 0;
                    $bulk_discount_products = $this->db->select('bulk_discount_min_qty, bulk_discount_amount')
                    ...
                    if (!empty($bulk_discount_products)) {
                        foreach ($bulk_discount_products as $bulk_product) {
                            if ($total_quantity >= $bulk_product['bulk_discount_min_qty']) {
                                $bulk_discount = max($bulk_discount, $bulk_product['bulk_discount_amount']);
                            }
                        }
                    }
