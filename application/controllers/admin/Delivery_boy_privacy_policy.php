...
class Delivery_boy_privacy_policy extends CI_Controller
{
    ...
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (print_msg(!has_permissions('update', 'delivery_boy_policies'), PERMISSION_ERROR_MSG, 'delivery_boy_policies')) {
                return false;
            }
            ...
            if (!$this->form_validation->run()) {
                ...
                $delivery_boy_data = array(
                    'privacy_policy_input_description' => $this->input->post('privacy_policy_input_description', true),
                );

                $delivery_boy_terms_n_condtions_data = array(
                    'terms_n_conditions_input_description' => $this->input->post('terms_n_conditions_input_description', true),
                );

                $this->Setting_model->update_delivery_boy_privacy_policy($delivery_boy_data);
                $this->Setting_model->update_delivery_boy_terms_n_condtions($delivery_boy_terms_n_condtions_data);
