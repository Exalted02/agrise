...
class Delivery_boys extends CI_Controller
{
    ...
    public function add_delivery_boy()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $edit_delivery_boy = $this->input->post('edit_delivery_boy', true);
            if (isset($edit_delivery_boy)) {
            ...
            if (!$this->input->post('edit_delivery_boy', true)) {
            ...
            $this->form_validation->set_rules('status', 'Status', 'trim|required|xss_clean|in_list[0,1]', array(
                'required' => 'Please select a status (Approved or Not-Approved)',
            ...
            $bonus_type = $this->input->post('bonus_type', true);
            if (isset($bonus_type) && $bonus_type == 'fixed_amount_per_order') {
            ...
            } elseif (isset($bonus_type) && $bonus_type == 'percentage_per_order') {
            ...
            $edit_delivery_boy = $this->input->post('edit_delivery_boy', true);
            if (isset($edit_delivery_boy)) {
                $delivery_boy_data = fetch_details('users', ['id' => $this->input->post('edit_delivery_boy', true)], 'driving_license');
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($files['driving_license']) && !empty($files['driving_license']['name'][0]) && isset($files['driving_license']['name'][0])) {
                    ...
                    if (isset($edit_delivery_boy) && !empty($edit_delivery_boy) && isset($delivery_boy_data[0]['driving_license']) && !empty($delivery_boy_data[0]['driving_license'])) {
                ...
                if (isset($edit_delivery_boy)) {
                    ...
                    $data = array(
                        ...
                        'bonus_amount' => $this->input->post('bonus_amount', true),
                    );


                    $this->Delivery_boy_model->update_delivery_boy($data);
                } else {
                ...
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_delivery_boy', true)) ? 'Delivery Boy Updated Successfully' : 'Delivery Boy Added Successfully';
    ...
    public function manage_cash_collection()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $message = (!empty($message)) ? $message : "Delivery boy cash collection by admin";
    ...
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $this->form_validation->set_rules('user_id', 'Delivery Boy', 'trim|required|xss_clean|numeric');
            if (!$this->form_validation->run()) {
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $delivery_boy_id = $this->input->post('user_id', true);
                $order =   $this->db->select('id as order_id , final_total , ')->where(['delivery_boy_id' => 5454353 , 'active_status' => 'delivered' ])->get('orders')->result_array();
                
                if (empty($order)) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'No Orders found for selected delivery boy ';
                    echo json_encode($this->response);
                    return false;
                }
            ...
            $this->response['csrfName'] = $this->security->get_csrf_token_name();
            $this->response['csrfHash'] = $this->security->get_csrf_hash();
            $this->response['message'] = 'Un-authorized Action';
            echo json_encode($this->response);
            return false;
        }
    }
