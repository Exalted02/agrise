    ...
    public function update_contact_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $data = array(
                    'contact_us' => $this->input->post('contact_us', true),
                    'contact_input_description' => $this->input->post('contact_input_description', true)
                );
                $this->Setting_model->update_contact_details($data);
