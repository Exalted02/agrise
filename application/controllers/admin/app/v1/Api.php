            ...
            if (empty($delivery_boy)) {
                ...
                if (isset($user_res[0]) && !empty($user_res[0])) {
                    ...
                    if (isset($current_delivery_boy[0]['delivery_boy_id']) && $current_delivery_boy[0]['delivery_boy_id'] == $this->input->post('delivery_boy_id', true)) {
                        ...
                        $data = str_replace(array($hashtag_customer_name, $hashtag_order_id, $hashtag_application_name), array($user_res[0]['username'], $_POST['orderid'], $app_name), $hashtag);
                        $notification_message = output_escaping(trim($data, '"'));

                        $customer_msg = (!empty($custom_notification)) ? $notification_message :  'Hello Dear ' . $user_res[0]['username'] . ' order status updated to ' . $_POST['status'] . ' for order ID #' . $_POST['order_id'] . ' assigned to you please take note of it! Thank you. Regards ' . $app_name . '';
                        ...
                        $customer_msg = (!empty($custom_notification)) ? $notification_message : 'Hello Dear ' . $user_res[0]['username'] . ' you have new order to be deliver order ID #' . $_POST['order_id'] . ' please take note of it! Thank you. Regards ' . $app_name . '';
            ...
            if ($update_status == 1) {
                ...
                if ($priority_status[$this->input->post('status', true)] > $priority_status[$current_orders_status]) {
                    ...
                    if ($error == false) {
                        ...
                        $customer_res = fetch_details('users', ['id' => $user_id],  'username,fcm_id');
                        ...
                        if (!empty($customer_res)) {
                            ...
                            if (!empty($custom_notification) && isset($custom_notification[0]['message'])) {
                                ...
                                $data = str_replace(array($hashtag_customer_name, $hashtag_order_id, $hashtag_application_name), array($customer_res[0]['username'], $_POST['order_id'], $app_name), $hashtag);
                                $customer_notification_message = output_escaping(trim($data, '"'));
                            } else {
                                $customer_notification_message = '';
                            }

                            $customer_msg = (!empty($custom_notification)) ? $customer_notification_message :  'Hello Dear ' . $customer_res[0]['username'] . ' order status updated to ' . $_POST['status'] . ' for your order ID #' . $_POST['order_id'] . ' please take note of it! Thank you for shopping with us. Regards ' . $app_name . '';

                            $fcm_ids = array();
                            if (!empty($customer_res[0]['fcm_id'])) {
                                $fcmMsg = array(
                                    'title' => (!empty($custom_notification) && isset($custom_notification[0]['title'])) ? $custom_notification[0]['title'] : "Order status updated",
                                    'body' => $customer_msg,
                                    'type' => "order"
                                );
