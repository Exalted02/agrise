...
class Custom_notification extends CI_Controller
{
    ...
    {
        $edit_notification = $this->input->post('edit_notification', true);
        if (null !== $edit_notification) {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {   
                ...
                if (null !== $edit_custom_notification) {
                    if (is_exist(['type' => $this->input->post('type', true)], 'custom_notifications', $this->input->post('edit_custom_notification', true))) {
                        $response["error"]   = true;
                    ...
                    if (!$this->form_validation->is_unique($this->input->post('type', true), 'custom_notifications.type')) {
                        $response["error"]   = true;
                        $response["message"] = "Name Already Exist ! Provide a unique type";
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $this->custom_notification_model->add_custom_notification($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_custom_notification', true)) ? 'Notification Updated Successfully' : 'Notification Added Successfully';
