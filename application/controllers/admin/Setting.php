...
class Setting extends CI_Controller
{
    ...
    public function update_system_settings()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('delivery_charge', 'Delivery charge', 'trim|required|numeric|greater_than_equal_to[0]|xss_clean');
            $this->form_validation->set_rules('min_amount', 'Minimum amount', 'trim|required|numeric|greater_than_equal_to[0]|xss_clean');
            ...
            $this->form_validation->set_rules('platform_fees', 'Platform Fees', 'trim|required|numeric|greater_than_equal_to[0]|xss_clean');
            $this->form_validation->set_rules('low_stock_limit', 'Low stock limit', 'trim|numeric|greater_than[0]|xss_clean');
            ...
            $this->form_validation->set_rules('admin_store_state', 'admin_store_state', 'trim|required|xss_clean');
            ...
            $this->form_validation->set_rules('min_cod_order_amount', 'Minimum COD Order Amount', 'trim|numeric|greater_than_equal_to[0]|xss_clean');
            $this->form_validation->set_rules('max_cod_order_amount', 'Maximum COD Order Amount', 'trim|numeric|greater_than_equal_to[0]|xss_clean');
            ...
            $this->form_validation->set_rules('email_login', 'Email Login', 'trim|xss_clean');
            $this->form_validation->set_rules('whatsapp_number', 'WhatsApp Number', 'trim|numeric|greater_than[0]|xss_clean');
            ...
            $isReferEarnOn = $this->input->post('is_refer_earn_on', true);
            if (isset($isReferEarnOn) && $isReferEarnOn) {
                ...
                $this->form_validation->set_rules('refer_earn_bonus', 'Refer & Earn Bonus', 'trim|required|numeric|greater_than[0]|xss_clean');
                $this->form_validation->set_rules('refer_earn_method', 'Refer Earn method', 'trim|required|xss_clean');
                $this->form_validation->set_rules('max_refer_earn_amount', 'Maximum Refer & Earn Bonus', 'trim|required|numeric|greater_than[0]|xss_clean');
                $this->form_validation->set_rules('refer_earn_bonus_times', 'Refer & Earn Bonus times', 'trim|required|numeric|greater_than[0]|xss_clean');
            }
            $welcome_wallet_balance_on = $this->input->post('welcome_wallet_balance_on', true);
            if (isset($welcome_wallet_balance_on) && $welcome_wallet_balance_on) {
                $this->form_validation->set_rules('wallet_balance_amount', 'Welcome Wallet Balance Amount', 'trim|required|numeric|greater_than[0]|xss_clean');
            }

            $pincodeWiseDeliverability = $this->input->post('pincode_wise_deliverability', true);
            $cityWiseDeliverability = $this->input->post('city_wise_deliverability', true);
            if (!isset($pincodeWiseDeliverability) && !isset($cityWiseDeliverability)) {
            ...
            if (isset($cityWiseDeliverability)) {
                $this->form_validation->set_rules('global_delivery_charge_on_city', 'Global delivery charge on City', 'trim|required|numeric|greater_than_equal_to[0]|xss_clean');
                $this->form_validation->set_rules('global_free_delivery_amount_on_city', 'Global Free Delivery Amount on City', 'trim|required|numeric|greater_than_equal_to[0]|xss_clean');
            }

            $pincode_wise_deliverability = $this->input->post('pincode_wise_deliverability', true);
            $city_wise_deliverability = $this->input->post('city_wise_deliverability', true);
            if (isset($pincode_wise_deliverability) && isset($city_wise_deliverability)) {
                $this->response['error'] = true;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
            ...
            $min_cod_amount = $this->input->post('min_cod_order_amount', true);
            $max_cod_amount = $this->input->post('max_cod_order_amount', true);
            if (!empty($min_cod_amount) && !empty($max_cod_amount)) {
                if ((float)$min_cod_amount >= (float)$max_cod_amount) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = 'Minimum COD order amount must be less than maximum COD order amount';
                    print_r(json_encode($this->response));
                    return false;
                }
            }
            if (!$this->form_validation->run()) {
                ...
                $_POST['system_timezone_gmt'] = preg_replace('/\s+/', '', $this->input->post('system_timezone_gmt', true));
                $_POST['whatsapp_number'] =  $this->input->post('whatsapp_number', true);
                $_POST['copyright_details'] =  $this->input->post('copyright_details', true);
                $_POST['system_timezone_gmt'] = ($this->input->post('system_timezone_gmt', true) == '00:00') ? "+" . $this->input->post('system_timezone_gmt', true) : $this->input->post('system_timezone_gmt', true);
    ...
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (print_msg(!has_permissions('update', 'web_settings'), PERMISSION_ERROR_MSG, 'settings')) {
                return false;
            }
