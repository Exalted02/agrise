...
class Flash_sale extends CI_Controller
{
    ...
    public function add_flash_sale()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $edit_flash_sale = $this->input->post('edit_flash_sale', true);
            if (isset($edit_flash_sale)) {
            ...
            $image_checkbox = $this->input->post('image_checkbox', true);
            $is_image_included = ($image_checkbox && $image_checkbox == 'on') ? TRUE : FALSE;
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($this->input->post('start_date', true) > $this->input->post('end_date', true)) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $this->Flash_sale_model->add_flash_sale($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (!empty($edit_flash_sale)) ? 'Sale Updated Successfully' : 'Sale Added Successfully';
