...
class Area extends CI_Controller
{
    ...
    public function add_area()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $edit_area = $this->input->post('edit_area', true);
            if (null !== $edit_area) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (null !== $edit_area) {
                    if (is_exist(['name' => $this->input->post('area_name', true), 'city_id' => $this->input->post('city', true), 'zipcode_id' => $this->input->post('zipcode', true)], 'areas', $this->input->post('edit_area', true))) {
                        $response["error"]   = true;
                    ...
                    if (is_exist(['name' => $this->input->post('area_name', true), 'city_id' => $this->input->post('city', true),  'zipcode_id' => $this->input->post('zipcode', true)], 'areas')) {
                        $response["error"]   = true;
                        $response["message"] = "Combination Already Exist ! Provide a unique Combination";
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $data = array(
                    ...
                    'minimum_free_delivery_order_amount' => $this->input->post('minimum_free_delivery_order_amount', true),
                    'delivery_charges' => $this->input->post('delivery_charges', true),
                );
                $this->Area_model->add_area($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_area', true)) ? 'Area Updated Successfully' : 'Area Added Successfully';
    ...
    public function add_city()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('delivery_charges', ' Delivery Charges ', 'trim|required|numeric|xss_clean');
            $settings = get_settings('system_settings', true);
            if (isset($settings['city_wise_deliverability']) && $settings['city_wise_deliverability'] == 1) {
                $this->form_validation->set_rules('city_name', ' City Name ', 'trim|required|xss_clean');
            }
            ...
            if (!$this->form_validation->run()) {
                ...
                if (null !== $edit_city) {
                    if (is_exist(['name' => $this->input->post('city_name', true)], 'cities', $this->input->post('edit_city', true))) {
                        $response["error"] = true;
                        $response["message"] = "City Name Already Exists! Provide a unique name";
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                        $response["data"] = array();
                        echo json_encode($response);
                    ...
                    if (is_exist(['name' => $this->input->post('city_name', true)], 'cities')) {
                        $response["error"] = true;
                        $response["message"] = "City Name Already Exists! Provide a unique name";
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $this->Area_model->add_city($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_city', true)) ? 'City Updated Successfully' : 'City Added Successfully';
                $this->response['message'] = $message;
                print_r(json_encode($this->response));
            }
        } else {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($settings['pincode_wise_deliverability']) && $settings['pincode_wise_deliverability'] == 1) {
                    ...
                    $edit_zipcode = $this->input->post('edit_zipcode', true);
                    if (null !== $edit_zipcode) {
                        if (is_exist(['city_id' => $this->input->post('city', true), 'zipcode' => ''], 'zipcodes', $this->input->post('edit_zipcode', true))) {
                            $response["error"]   = true;
                            $response["message"] = "Combination Already Exist ! Provide a unique Combination";
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                        ...
                        if (is_exist(['city_id' => $this->input->post('city', true), 'zipcode' => ''], 'zipcodes')) {
                            $response["error"]   = true;
                            $response["message"] = "Combination Already Exist ! Provide a unique Combination";
                            $response['csrfName'] = $this->security->get_csrf_token_name();
                            $response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $this->Area_model->add_zipcode($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (null !== $this->input->post('edit_zipcode', true)) ? 'Zipcode Updated Successfully' : 'Zipcode Added Successfully';
                $this->response['message'] = $message;
                print_r(json_encode($this->response));
            }
        } else {
    ...
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (print_msg(!has_permissions('create', 'zipcodes'), PERMISSION_ERROR_MSG, 'product')) {
                return false;
            }
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($type == 'upload' && $location_type == 'zipcode') {
                    if (!has_permissions('create', 'zipcodes')) {
                        $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                        redirect('admin/home', 'refresh');
                    }
                    ...
                    return false;
                                        ...
                                        if (!has_permissions('create', 'city')) {
                        $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                        redirect('admin/home', 'refresh');
                    }
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) {
                        if ($temp != 0) {
                            ...
                            if (empty($row[2])) {
                                $this->response['error'] = true;
                                $this->response['message'] = 'Delivery Charges is empty at row ' . $temp;
                                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                                print_r(json_encode($this->response));
                                return false;
                    ...
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) {
                        if ($temp1 != 0) {
                            $data['name'] = $row[0];
                            $data['minimum_free_delivery_order_amount'] = $row[1];
                            $data['delivery_charges'] = $row[2];
                    ...
                    return false;
                                        ...
                                        if (!has_permissions('create', 'area')) {
                        $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                        redirect('admin/home', 'refresh');
                    }
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) {
                    ...
                    return false;
                                        ...
                                        if (!has_permissions('update', 'zipcodes')) {
                        $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                        redirect('admin/home', 'refresh');
                    }
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) {
                    ...
                    return false;
                                        ...
                                        if (!has_permissions('update', 'area')) {
                        $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                        redirect('admin/home', 'refresh');
                    }
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) {
    ...
    {
        if (!has_permissions('update', 'zipcodes')) {
            print_msg(PERMISSION_ERROR_MSG, PERMISSION_ERROR_MSG);
            redirect('admin/home', 'refresh');
        }
        $table_name = 'zipcodes';

        $columns_to_check = array('city_id', 'minimum_free_delivery_order_amount', 'delivery_charges');
        ...
        if (!empty($area_data)) {
            ...
            $response_data['error'] =  true;
            $response_data['message'] =  'No area data available to sync. Please add area data first.';
            print_r(json_encode($response_data));
        }
    }
