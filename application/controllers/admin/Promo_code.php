...
class Promo_code extends CI_Controller
{
    ...
    public function add_promo_code()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('end_date', 'End date', 'trim|required|xss_clean|callback_check_dates');
            ...
            if ($this->input->post('discount_type', true) == 'amount') {
                ...
                $this->form_validation->set_rules('discount', 'Discount', 'trim|required|numeric|xss_clean|less_than_equal_to[100]');
                $this->form_validation->set_rules('max_discount_amount', 'Max Discount Amount', 'trim|numeric|xss_clean');
            }
