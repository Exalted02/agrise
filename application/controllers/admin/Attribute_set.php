    ...
    public function add_attribute_set()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $edit_attribute_set = $this->input->post('edit_attribute_set', true);
                if (null !== $edit_attribute_set) {
                    if (is_exist(['name' => $this->input->post('name', true)], 'attribute_set', $this->input->post('edit_attribute_set', true))) {
                        $response["error"]   = true;
                    ...
                    if (!$this->form_validation->is_unique($this->input->post('name', true), 'attribute_set.name')) {
                        $response["error"]   = true;
                        $response["message"] = "Name Already Exist ! Provide a unique name";
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
                ...
                $this->attribute_model->add_attribute_set($data);
                $this->response['error'] = false;
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $this->response['message'] = 'Attribute Added Successfully';
                $message = (null !== $this->input->post('edit_attribute_set', true)) ? 'Attribute Set Updated Successfully' : 'Attribute Set Added Successfully';
