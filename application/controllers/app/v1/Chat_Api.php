...
*/
        ...
        */
            ...
            if (!$this->form_validation->run()) {
                ...
                $to_id = (isset($_POST['to_id'])) ? $this->input->post('to_id', true) : $this->user_details['id'];
                $type = (isset($_POST['type'])) ? $this->input->post('type', true) : 'person';
        ...
        */
            ...
            if (!$this->form_validation->run()) {
                ...
                if (!$msg_id) {
                    $response['error'] = true;
                    $response['message'] = 'Failed to send message. Please check server logs for details.';
                    $response['data'] = array();
                    print_r(json_encode($response));
                    return;
                }
                ...
                // Now retrieve the message details for response
                ...
                // Only process notifications if we can retrieve message details
                if ($messages !== false && !empty($messages)) {
                    ...
                    // Message was sent successfully, now handle notifications
                    // single user msg
                    if ($type == 'person') {
                    ...
                    // If we couldn't retrieve message details, still return success but without new_msg
                    $response['new_msg'] = array();
                }
            }
            print_r(json_encode($response));
        }
