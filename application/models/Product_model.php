...
class Product_model extends CI_Model
{
    ...
    public function add_product($data)
    {
        ...
        $bulk_discount_min_qty = (isset($data['bulk_discount_min_qty']) && !empty($data['bulk_discount_min_qty'])) ? $data['bulk_discount_min_qty'] : 0;
        $bulk_discount_amount = (isset($data['bulk_discount_amount']) && !empty($data['bulk_discount_amount'])) ? $data['bulk_discount_amount'] : 0;
        ...
        $pro_data = [
            ...
            'bulk_discount_min_qty' => $bulk_discount_min_qty,
            ...
            'deliverable_type' => isset($data['deliverable_type']) && !empty($data['deliverable_type']) ? $data['deliverable_type'] : 1,
            'deliverable_city_type' => isset($data['deliverable_city_type']) && !empty($data['deliverable_city_type']) ? $data['deliverable_city_type'] : 1,
        ...
        if ($pro_type == 'simple_product') {
            $pro_variance_data = [
                ...
                'weight' => (isset($data['weight']) && !empty($data['weight'])) ? $data['weight'] : '0.0',
                'height' => (isset($data['height']) && !empty($data['height'])) ? $data['height'] : '0.0',
                'breadth' => (isset($data['breadth']) && !empty($data['breadth'])) ? $data['breadth'] : '0.0',
                'length' => (isset($data['length']) && !empty($data['length'])) ? $data['length'] : '0.0',
            ];
    ...
    public function get_product_details($flag = NULL)
    {
        ...
        $search_res = $this->db->select('product_variants.id AS id, p.id as pid ,p.rating,p.no_of_ratings,p.name, p.type, p.image, p.status,product_variants.price , product_variants.special_price,p.brand, product_variants.stock, product_variants.availability ')
        ...
        foreach ($pro_search_res as $row) {
            ...
            $product_details = fetch_product("", null, $row['pid']);
            $product = isset($product_details['product'][0]) ? $product_details['product'][0] : null;

            // Build product name with type
            $product_name = $row['name'] . ' ' . ' <small>(' . ucwords(str_replace('_', ' ', $row['type'])) . ')</small>';
            ...
            if ($product) {
                ...
                foreach ($variants as $variant) {
                    if ($variant['stock'] == 0 || $variant['availability'] == '0') {
                ...
                if ($is_out_of_stock) {
                    $product_name .= ' <span class="badge bg-danger text-white ml-2">Out of Stock</span>';
                }
            }
            $tempRow['name'] = $product_name;
        ...
        if (isset($_GET['sort']) && $_GET['sort'] == 'variations') {
            $order = (isset($_GET['order']) && strtolower($_GET['order']) == 'asc') ? 'asc' : 'desc';

            usort($rows, function ($a, $b) use ($order) {
                $a_val = strip_tags($a['variations']);
                $b_val = strip_tags($b['variations']);

                return ($order === 'asc') ? strcasecmp($a_val, $b_val) : strcasecmp($b_val, $a_val);
            });
        }
    ...
    function get_offer_brands($search_term = "")
    {
        ...
        $this->db->select('*');
        $this->db->where("name like '%" . $search_term . "%'");
        $this->db->where("status", 1); // Add this line to filter by status
        $fetched_records = $this->db->get('brands');
        $brands = $fetched_records->result_array();
    ...
    public function get_brand_list($search = "", $offset = 0, $limit = 25)
    {
        $where = ['status' => 1];
        $multipleWhere = [];

        if (!empty($search)) {
            $multipleWhere = ['`name`' => $search];
        }

        $count_query = $this->db->from('brands');
        ...
        $search_res = $this->db->select('id, name, image')->from('brands');
        ...
        if (!empty($brands)) {
            foreach ($brands as &$brand) {
                ...
                $brand['relative_path'] = !empty($brand['image']) ? $brand['image'] : [];
    ...
    function get_product_faqs($id = '', $product_id = '', $user_id = '', $search = '', $offset = '0', $limit = '10', $sort = 'id', $order = 'DESC')
    {
        ...
        // Removed user_id filtering so all users can see all FAQs
        // if (!empty($user_id)) {
    ...
    public function get_faqs()
    {
        ...
        // Note: No else clause needed as $sort keeps its original value
    ...
    {
        // print_r($_GET);
        $filters['show_only_stock_product'] = 1;
        ...
        // print_r($filters);
        // Fetch all products without filtering by stock_type
        ...
        // $filtered_products = array_filter($products['product'], function ($product) {
        //     return isset($product['stock_type']) && $product['stock_type'] !== null && $product['stock_type'] !== '';
        // });

        // Update total to reflect the number of filtered products
        // $total = count($filtered_products);
        // $bulkData['total'] = $total;
        $filtered_products = $products['product'];
        ...
        foreach ($filtered_products as $product) {
            ...
            for ($i = 0; $i < count($variants); $i++) {
                ...
                $tempRow['current_stock'] = $variants[$i]['stock'];
                $operate .= "<tr> <th>" . str_replace(",", ", ", $variants[$i]['variant_values'])  . '</th>';
                if ($product['stock_type'] != 1) {
                    $stock_display = str_replace(",", ", ", $variants[$i]['stock']);
                    if ($variants[$i]['stock'] == 0 || $variants[$i]['availability'] == '0') {
                        $stock_display .= ' <span class="badge bg-danger text-white ml-1">Out of Stock</span>';
                    }
                    $operate .= '<td><b>' . $stock_display . '</b></td>';
                    $operate .= '<td><b>' . $edit  . '</b></td></tr>';
                } else {
                    if ($i == 0) {
                        $stock_display = $variants[$i]['stock'];
                        if ($variants[$i]['stock'] == 0 || $variants[$i]['availability'] == '0') {
                            $stock_display .= ' <span class="badge bg-danger text-white ml-1">Out of Stock</span>';
                        }
                        $operate .= '<td rowspan="' . count($variants) . '"><b>' .  $stock_display . '</b></td>';
            ...
            if ($product['availability'] == 0) {
                $tempRow['current_stock'] = '0';
                $operate .= "<tr><td colspan='2'><span class='badge bg-danger text-white'>Sold Out</span></td></tr>";
            }
            $operate .= "</table>";
            $simple_product_stock_display = ($product['stock']);
            if (isset($product['stock']) && $product['stock'] == 0) {
                $simple_product_stock_display .= ' <span class="badge bg-danger text-white ml-1">Out of Stock</span>';
            }
            $tempRow['operate'] = (isset($product['stock']) && !empty($product['stock'])) ? '<table class="table-borderless table-sm w-100"><tr><th><b>'  . 'Simple Product' . '</b></th><td> <b>'  . $simple_product_stock_display . '</b></td><td>' . ' ' . $edit . "</td></tr></table>" : $operate;
            $rows[] = $tempRow;
        }
        $bulkData['rows'] = $rows;
