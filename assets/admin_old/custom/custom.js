...
$(document).on('load-success.bs.table', '#products_table', function (event) {
    ...
    var $pageLinks = $pagination.find('li:not(.page-first):not(.page-pre):not(.page-next):not(.page-last)');
...
$(document).on('page-change.bs.table', '#products_table', function (event) {
    setTimeout(function () {
        var $pagination = $('#products_table').closest('.bootstrap-table').find('.pagination');
        var $pageLinks = $pagination.find('li:not(.page-first):not(.page-pre):not(.page-next):not(.page-last)');
        var currentPage = $pagination.find('li.active').index();
        var totalPages = $pageLinks.length;
...
$(document).on('click', '.read-more-btn', function (e) {
    ...
    if (action === 'more') {
        $fullText.show();
        $commentText.removeClass('comment-text-truncated').addClass('comment-text-full');
        $this.text('Read Less').data('action', 'less');
    } else {
        $fullText.hide();
        $commentText.removeClass('comment-text-full').addClass('comment-text-truncated');
...
if (document.getElementById('piechart_3d')) {
    $.ajax({
        ...
        success: function (result) {
            ...
            // Render the chart
            var chart = new ApexCharts(document.getElementById('piechart_3d'), options);
            chart.render();
        }
    });
    ...
    $.ajax({
        ...
        success: function (result) {
                ...
                if (!labels || !data || labels.length === 0 || data.length === 0) {
                    ...
                    if (chartKey === 'week' && labels.length < 7) {
                        ...
                        for (var i = 0; i < 7; i++) {
                            ...
                            if (foundIndex >= 0 && i === Math.floor(7 / 2)) { // Place the data point in the middle
            ...
            function renderActiveChart() {
                ...
                switch (activeHref) {
                    case '#scoreLineToDay':
                        createLineChart('scoreLineToDay', dayData.labels, dayData.series, 'day');
                        break;
                    case '#scoreLineToWeek':
                        createLineChart('scoreLineToWeek', weekData.labels, weekData.series, 'week');
                        break;
                    case '#scoreLineToMonth':
                        createLineChart('scoreLineToMonth', monthData.labels, monthData.series, 'month');
                        break;
                }
            }
            ...
            // Handle tab clicks
            $(document).off('click.chartTabs', '.chart-action li a').on('click.chartTabs', '.chart-action li a', function (e) {
                e.preventDefault();
...
document.addEventListener('DOMContentLoaded', function () {
    const cityWiseDeliverability = document.getElementById('city_wise_deliverability');
    const cityDeliverySettings = document.querySelectorAll('.city-delivery-settings');
...
$('#fund_transfer').on(
    ...
    function (field, value, row, $el) {
        var balance = $el.balance ? $el.balance.toString().replace(/,/g, '') : $el.balance;
...
$('.add_product_form').on('click', function (e) {
    ...
    if (!status) {
        ...
        iziToast.error({
            message: 'Please select a status (Approved or Not-Approved)',
            position: 'topRight'
        });
        return false;
    } else {
...
$(document).on('change', '.swatche_type', function () {
    ...
    if ($(this).val() == '1') {
        ...
        imageUploadSection.find('input[type="hidden"]').attr('name', 'swatche_value[]');
    }
    else if ($(this).val() == '0') {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const minimumOrderQuantity = document.getElementById('minimum_order_quantity');
    const totalAllowedQuantity = document.getElementById('total_allowed_quantity');

    function validateStepSize() {
        const minOrderValue = minimumOrderQuantity && minimumOrderQuantity.value ? parseInt(minimumOrderQuantity.value, 10) : NaN;
        const stepSizeValue = quantityStepSize && quantityStepSize.value ? parseInt(quantityStepSize.value, 10) : NaN;
        const totalAllowedValue = totalAllowedQuantity && totalAllowedQuantity.value ? parseInt(totalAllowedQuantity.value, 10) : NaN;

        if (!isNaN(stepSizeValue)) {
            if (!isNaN(minOrderValue) && stepSizeValue < minOrderValue) {
                iziToast.error({
                    title: 'Error',
                    message: 'Quantity Step Size cannot be less than Minimum Order Quantity!',
                    position: 'topRight'
                });
            }
            ...
            if (!isNaN(totalAllowedValue)) {
                if (stepSizeValue > totalAllowedValue) {
                    iziToast.error({
                        title: 'Error',
                        message: 'Quantity Step Size cannot be greater than Total Allowed Quantity!',
                        position: 'topRight'
                    });
                }
                ...
                if (totalAllowedValue % stepSizeValue !== 0) {
                    iziToast.error({
                        title: 'Error',
                        message: 'Quantity Step Size must be a multiple of Total Allowed Quantity!',
                        position: 'topRight'
                    });
                }
            }
...
$('#submitPurchaseCode').click(function () {
    ...
    if (purchase_code == current_code) {
        ...
        iziToast.error({
            message: "Incorrect Purchase Code! Please confirm your purchase code and try again."
        });
    }
});
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.error == false) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: base_url + "admin/purchase_code/delete_app_purchase_code",
                                    data: { 'de_register_code': de_register },
                                    dataType: 'json',
                                    success: function (result) {
                                        if (result.error == false) {
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    Toast.fire({
                        icon: 'error',
                        title: 'Error deleting return reason: ' + error
                    });
                }
            });
        }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.error === false && response.data) {
                ...
                $('#inventory_pie_chart').html('<div class="text-center p-4"><p>No data available for the selected period</p></div>');
                $('#inventory_units_chart').html('<div class="text-center p-4"><p>No data available for the selected period</p></div>');
            }
        },
        error: function (xhr, status, error) {
...
function renderPieChart(data) {
    ...
    if (!data || data.length === 0) {
        $('#inventory_pie_chart').html('<div class="text-center p-4"><p>No data available for chart</p></div>');
        return;
    }

    var labels = data.map(function (item) { return item.product_name; });
    var series = data.map(function (item) { return parseFloat(item.final_total); });

    // Calculate total sales
    var totalSales = series.reduce(function (sum, value) { return sum + value; }, 0);
    ...
    var options = {
        ...
        chart: {
            ...
            events: {
                dataPointMouseEnter: function (event, chartContext, config) {
                    ...
                    var centerDiv = document.querySelector('#inventory_pie_chart .custom-center-content');
                    if (centerDiv) {
                        centerDiv.innerHTML = '<div class="center-label" style="font-size: 16px; font-weight: 600; color: #373d3f; margin-bottom: 5px;">' + truncatedLabel + '</div><div class="center-value" style="font-size: 22px; font-weight: 400; color: #373d3f;">$' + value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '</div>';
                    }
                },
                dataPointMouseLeave: function (event, chartContext, config) {
                    ...
                    var centerDiv = document.querySelector('#inventory_pie_chart .custom-center-content');
                    if (centerDiv) {
                        centerDiv.innerHTML = '<div class="center-label" style="font-size: 16px; font-weight: 600; color: #373d3f; margin-bottom: 5px;">Total Sales</div><div class="center-value" style="font-size: 22px; font-weight: 400; color: #373d3f;">$' + totalSales.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '</div>';
                    }
                }
            }
        },
        ...
        colors: ['#8e44adee', '#27ae60ee', '#e67e22ee', '#3498dbee', '#e74c3cee', '#f1c40fee', '#1abc9cee', '#ff69b4ee', '#7f8c8dee', '#9b59b6ee'],
        legend: {
            position: 'bottom',
    ...
    inventoryChart = new ApexCharts(document.querySelector("#inventory_pie_chart"), options);
    inventoryChart.render().then(function () {
        // Add custom center content overlay
        var chartContainer = document.querySelector('#inventory_pie_chart');
        var existingCenter = chartContainer.querySelector('.custom-center-content');
        if (!existingCenter) {
        ...
        $('#inventory_pie_chart').html('<div class="text-center p-4"><p>Error rendering chart</p></div>');
    });
}
...
function renderUnitsChart(data) {
    ...
    if (!data || data.length === 0) {
        $('#inventory_units_chart').html('<div class="text-center p-4"><p>No data available for chart</p></div>');
        return;
    }

    var labels = data.map(function (item) { return item.product_name; });
    var series = data.map(function (item) { return parseInt(item.quantity); });

    // Calculate total units
    var totalUnits = series.reduce(function (sum, value) { return sum + value; }, 0);
        ...
        chart: {
            ...
            events: {
                dataPointMouseEnter: function (event, chartContext, config) {
                    ...
                    // Find and update custom center content
                    var centerDiv = document.querySelector('#inventory_units_chart .custom-center-content');
                    if (centerDiv) {
                        centerDiv.innerHTML = '<div class="center-label" style="font-size: 16px; font-weight: 600; color: #373d3f; margin-bottom: 5px;">' + truncatedLabel + '</div><div class="center-value" style="font-size: 22px; font-weight: 400; color: #373d3f;">' + value.toLocaleString() + ' units</div>';
                    }
                },
                dataPointMouseLeave: function (event, chartContext, config) {
                    // Reset to total when mouse leaves
                    var centerDiv = document.querySelector('#inventory_units_chart .custom-center-content');
                    if (centerDiv) {
                        centerDiv.innerHTML = '<div class="center-label" style="font-size: 16px; font-weight: 600; color: #373d3f; margin-bottom: 5px;">Total Units</div><div class="center-value" style="font-size: 22px; font-weight: 400; color: #373d3f;">' + totalUnits.toLocaleString() + ' units</div>';
                    }
                }
            }
        },
    ...
    inventoryUnitsChart = new ApexCharts(document.querySelector("#inventory_units_chart"), options);
    inventoryUnitsChart.render().then(function () {
        // Add custom center content overlay
        ...
        $('#inventory_units_chart').html('<div class="text-center p-4"><p>Error rendering chart</p></div>');
    });
}
...
$(document).ready(function () {
    ...
    if ($('#inventory_datepicker').length) {
        ...
        $('#inventory_datepicker').on('cancel.daterangepicker', function (ev, picker) {
            $(this).val('');
            $('#start_date').val('');
            $('#end_date').val('');
        ...
        $('#inventory_datepicker').on('apply.daterangepicker', function (ev, picker) {
...
$(document).ready(function () {
    $('#base_price').on('input', function () {
        var value = $(this).val();
        if (value && !isNaN(value) && value.indexOf('e') === -1 && value.indexOf('E') === -1) {
    ...
    $(document).on('input', '.variant-base-price', function () {
        ...
        if (!basePrice || isNaN(basePrice) || basePrice.indexOf('e') !== -1 || basePrice.indexOf('E') !== -1) {
            return;
        }
    ...
    $(document).on('keypress', '.variant-base-price, .variant-special-price, #base_price, #special_price, input[name="variant_price[]"], input[name="variant_special_price[]"]', function (e) {
    ...
    $(document).on('paste', '.variant-base-price, .variant-special-price, #base_price, #special_price, input[name="variant_price[]"], input[name="variant_special_price[]"]', function (e) {
        var $this = $(this);
        setTimeout(function () {
            var value = $this.val();
