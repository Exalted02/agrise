...
function display_products(products) {
  ...
  for (i = 0; i < products.length; i++) {
    ...
    var firstVariant = variants.length > 0 ? variants[0] : null
    var variantDisplay = ''
    if (firstVariant && firstVariant.variant_values) {
      variantDisplay = ' (' + firstVariant.variant_values + ')'
    }
    ...
    // Only show dropdown if more than one variant
    if (variants.length > 1) {
      ...
      // Hidden select for single variant to maintain functionality
      var variant_price =
      ...
      display_products +=
        '<div class="mt-2">' +
        '<span class="h5 text-primary font-weight-bold">' + currency + ' ' + parseFloat(variant_price).toLocaleString() + '</span>' +
        '</div>' +
        '<select class="form-control product-variants variant_value" id="change" style="display:none;">' +
        '<option data-variant_values="' +
        ...
        '" value="' +
        variant_price +
        '" class="shop-item-price" selected> ' +
        (variants[0]['variant_values'] ? variants[0]['variant_values'] + ' - ' : '') +
        currency +
        ' ' +
        parseFloat(variant_price).toLocaleString() +
        '</option>' +
    ...
    display_products +=
      ' <div class="shop-item-details justify-content-center ' + (variants.length === 1 ? 'mt-2' : '') + '">' +
...
function paginate(total, current_page, limit) {
  // Only show pagination if there are results and pagination is needed
  if (!total || total <= 15 || total <= limit) {
