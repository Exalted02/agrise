...
$(document).ready(function () {
    ...
    $("input[name='payment_method']").on('change', function (e) {
        ...
        $('.delivery-charge').html(delivery_charge.toLocaleString(undefined, {
            ...
            success: function (result) {
                ...
                if (product_type === 'digital_product' || local_pick_up == '1') {
                    ...
                    if (selectedPaymentMethod === 'COD') {
                        ...
                        delivery_charge_with_cod_val = (delivery_charge_with_cod_val != null) ? String(delivery_charge_with_cod_val) : '';
                        delivery_charge_with_cod_val = delivery_charge_with_cod_val.replace(/,/g, '');
                        delivery_charge = parseFloat(delivery_charge_with_cod_val) || 0;
                    } else {
                        var delivery_charge_without_cod_val = result.delivery_charge_without_cod;
                        delivery_charge_without_cod_val = (delivery_charge_without_cod_val != null) ? String(delivery_charge_without_cod_val) : '';
                        delivery_charge_without_cod_val = delivery_charge_without_cod_val.replace(/,/g, '');
...
$(document).ready(function () {
    ...
    var initial_bulk_discount = $('#bulk_discount').val() ? parseFloat($('#bulk_discount').val()) : 0;
    var initial_platform_fees = $('#platform_fees').val() ? parseFloat($('#platform_fees').val()) : 0;
    
    if (initial_bulk_discount > 0 && initial_sub_total) {
        var initial_total = parseFloat(initial_sub_total) + initial_platform_fees - initial_bulk_discount;
        $('#final_total').html(initial_total.toLocaleString(undefined, {
            maximumFractionDigits: 2
        }));
    }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var platform_fees = $('#platform_fees').val() ? parseFloat($('#platform_fees').val()) : 0;
            var bulk_discount = $('#bulk_discount').val() ? parseFloat($('#bulk_discount').val()) : 0;
            var sub_total_val = sub_total ? parseFloat(sub_total.replace(/,/g, '')) : 0;
            
            var final_total = sub_total_val + delivery_charge + platform_fees - bulk_discount;
            console.log(final_total);
            console.log('sub_total', sub_total);
            console.log('delivery_charge', parseFloat(delivery_charge));
...
$(document).on('click', '#wallet_balance', function () {
    ...
    if ($(this).is(':checked')) {
        ...
        if (is_cashback == 1) {
            final_total = parseFloat(sub_total) + parseFloat(delivery_charge) + parseFloat(platform_fees) - parseFloat(bulk_discount);
        } else {
            final_total = parseFloat(sub_total) + parseFloat(delivery_charge) - parseFloat(promocode_amount) + parseFloat(platform_fees) - parseFloat(bulk_discount);
        }
