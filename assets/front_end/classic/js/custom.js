...
// Password toggle functionality using jQuery event delegation
$(document).on('click', '.togglePassword', function () {
    // Try different methods to find the password input
    let input = $(this).prev()[0]; // Method 1: Direct sibling using jQuery
    
    // Method 2: If direct sibling doesn't work, look within the same input-group
    if (!input || (input.type !== 'password' && input.type !== 'text')) {
        const inputGroup = $(this).closest('.input-group');
        if (inputGroup.length) {
            input = inputGroup.find('input[type="password"], input[type="text"]')[0];
        }
    }
    ...
    if (!input || (input.type !== 'password' && input.type !== 'text')) {
        ...
        input = parent.find('input[type="password"], input[type="text"]')[0];
    }
    
    // If we found a valid input, toggle it
    if (input && (input.type === 'password' || input.type === 'text')) {
        const icon = $(this).find('i'); // Get the eye icon
...
$(document).on('submit', '.form-submit-event', function (e) {
    ...
    // Handle email login
    if ($(this).attr('id') === 'login_form' && $(this).data('login-mode') === 'email') {
        e.preventDefault();

        var type = 'email'
        // Get email from the email form's identity field
        var email = $('#login-email-form input[name="identity"]').val();
    ...
    // Handle email login for register page
    if ($(this).attr('id') === 'login_form_email') {
        e.preventDefault();

        var type = 'email'
        var email = $(this).find('input[name="identity"]').val();
        var password = $(this).find('input[name="password"]').val();
...
// Email/Phone login toggle functionality for classic theme modals
$(document).on('click', '#login-canvas-emailLogin', function (e) {
    e.preventDefault();
    ...
    // Remove required attribute from mobile input and add to email input
    $('#login-mobile-form input[name="identity"]').removeAttr('required');
    $('#login-email-form input[name="identity"]').attr('required', 'required');
    ...
    $('#login_form').validate({
        ignore: ":hidden", // Ignore hidden form elements
        rules: {
            identity: {
                required: true,
                email: true
...
// Handle phone login button click in modal
$(document).on('click', '#login_div #phoneLogin', function (e) {
    e.preventDefault();
    ...
    // Remove required attribute from email input and add to mobile input
    $('#login-email-form input[name="identity"]').removeAttr('required');
    $('#login-mobile-form input[name="identity"]').attr('required', 'required');
...
$(document).on('click', '#register_div #emailLogin', function (e) {
    e.preventDefault();
...
// Handle phone registration button click
$(document).on('click', '#register_div #phoneLogin', function (e) {
    e.preventDefault();
...
// Handle email signup form submission for classic theme
$(document).on('submit', '#sign-up-form', function (e) {
    e.preventDefault();
            ...
            if (result.error == true) {
                ...
                firebase.auth().createUserWithEmailAndPassword(email, password)
                    ...
                    .catch(function (error) {
                        ...
                        $('#sign-up-error').html('<div class="alert alert-danger">Registration failed: ' + error.message + '</div>');
                    });
            } else {
                $('#sign-up-error').html('<div class="alert alert-danger">' + result.message + '</div>');
            }
...
// Initialize form validation for classic theme
$(document).ready(function() {
    // Set up initial validation for login form (mobile number by default)
    $("#login_form").validate({
        ignore: ":hidden", // Ignore hidden form elements
        rules: {
            identity: {
    ...
    if (cart !== null && cart.length > 0) {
        ...
        cart.forEach(item => {
            ...
            html +=
                ...
                currency +
                ' ' +
                (item.qty * item.special_price).toLocaleString(undefined, {
        ...
        success: function (result) {
            ...
            if (result.error == false) {
                if (compare !== null && compare_count > 0) {
                    ...
                    comp +=
                        '<tr>' +
                        '<th class="compare-field text-center text-justify">Guarantee</th>'
                    $.each(result.data.product, function (i, e) {
                        comp +=
                            '<td class="text-center text-justify" data-title="warranty period">' +
    ...
    if (filters != '') {
        ...
        $.each(JSON.parse(filters), function (i, e) {
            ...
            html +=
                ...
                '<h2 class="clearfix mb-0">' +
                '<a class="collapse-arrow btn btn-link collapsed" data-toggle="collapse" data-target="#' +
                prefix + i + '" aria-expanded="true" aria-controls="#' +
                prefix + i + '">' +
            ...
            $.each(attribute_values, function (j, v) {
                ...
                html +=
                    '<div class="input-container d-flex">' +
                    '<input type="checkbox" name="' + decodedValue + '" value="' + decodedValue + '" class="toggle-input product_attributes" id="' +
                    prefix + new_attr_val + '" data-attribute="' + e_name + '" ' + attr_checked_status + '>' +
                    '<label class="toggle checkbox" for="' + prefix + new_attr_val + '">' +
                    '<div class="toggle-inner"></div>' +
                    '</label>' +
                    '<label for="' + prefix + new_attr_val + '" class="text-label">' + decodedValue + '</label></div>';
            });
            html += '</div></div></div>';
        });
    }
...
function print_filters_mobile(filters, prefix = '', target) {
    ...
    if (filters) {
        $.each(JSON.parse(filters), function (i, e) {
            const e_name = decodeUnicode(e.name.replace(' ', '-').toLowerCase());
            const attr_name = getUrlParameter('filter-' + e_name);
            const collapse_status = attr_name ? 'show' : '';
            html += `
                <div class="nav flex-column nav-pills" id="${prefix}${i}-tab1" role="tablist" aria-orientation="vertical">
                    <button class="nav-link filter-titles" id="${prefix}${i}-tab" data-bs-toggle="tab" data-bs-target="#${prefix}${i}" type="button" role="tab" aria-controls="${prefix}${i}" aria-selected="true">
                        ${e.name}
                    </button>
                </div>`;
        });
...
function print_filters_mobile_value(filters, prefix = '', target) {
    ...
    if (filters) {
        $.each(JSON.parse(filters), function (i, e) {
            ...
            const attr_name = getUrlParameter('filter-' + e_name);
            const collapse_status = attr_name ? 'show' : '';
            const selected_attributes = attr_name ? attr_name.split('|') : [];

            html += `
                <div class="tab-pane fade ${collapse_status}" id="${prefix}${i}" role="tabpanel" aria-labelledby="${prefix}${i}-tab" tabindex="0">
                    <div class="form-check">`;

            const attribute_values_id = e.attribute_values_id.split(',');
            const attribute_values = e.attribute_values.split(',');

            $.each(attribute_values, function (j, v) {
                const attr_checked_status = selected_attributes.includes(v) ? 'checked' : '';
                const new_attr_val = `${e_name} ${v}`;
                html += `
                    <label class="form-check-label py-2" for="${prefix}${new_attr_val}">
                        <input type="checkbox" name="${v}" value="${v}" class="toggle-input product_attributes width15px form-check-input" id="${prefix}${new_attr_val}" data-attribute="${e_name}" ${attr_checked_status}>
                        ${v}
                    </label>`;
            });
            html += '</div></div>';
...
//                                     media_files += '<span class="chat-image-view d-grid">' +
...
// Email verification and password reset functionality for classic theme
$(document).ready(() => {
    // Forgot password email functionality for classic theme
    ...
    if (window.location.pathname.includes("register")) {
        ...
        // Initialize email/phone form flags
    let isEmailLoginMode = false;
    
    // Toggle between email and phone login
    $('#login-emailLogin').on('click', function(e) {
        e.preventDefault();
    ...
    $('#login-phoneLogin').on('click', function(e) {
        e.preventDefault();
