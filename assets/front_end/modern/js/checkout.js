...
$(document).ready(function () {
    ...
    // Initially hide the bank transfer details
    $('#bank_transfer_slide').hide();

    // Handle click event on bank transfer radio button
    $('#bank_transfer').change(function () {
        if ($(this).is(':checked')) {
            $('#bank_transfer_slide').slideDown(); // Show the bank transfer details
        } else {
            $('#bank_transfer_slide').slideUp(); // Hide the bank transfer details
        }
    });
});
$(document).ready(function () {
    ...
    $("input[name='payment_method']").on('change', function (e) {
        ...
        let delivery_charge_with_cod = $('.delivery_charge_with_cod').val()?.replace(',', '') || '0';
        let delivery_charge_without_cod = $('.delivery_charge_without_cod').val()?.replace(',', '') || '0';
        let sub_total = $('#sub_total').val();
        let delivery_charge = 0;
        sub_total = sub_total.replace(',', '') || '0';
        let promocode_amount = $('#promocode_amount').text();
        ...
        if (promocode_amount == '') {
            ...
            promocode_amount = promocode_amount.replace(',', '') || '0';
        }
        ...
        if (wallet_used == '') {
            ...
            wallet_used = wallet_used.replace(',', '') || '0';
        }
        ...
        if (product_type === 'digital_product' || local_pick_up == '1') {
            ...
            if (payment_method == "COD") {
                delivery_charge = isNaN(parseFloat(delivery_charge_with_cod)) ? 0 : parseFloat(delivery_charge_with_cod);
            } else {
                delivery_charge = isNaN(parseFloat(delivery_charge_without_cod)) ? 0 : parseFloat(delivery_charge_without_cod);
            }
    ...
    $('.address-modal').on('click', '.submit', function (event) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                let platform_fees = $('#platform_fees').val() ? parseFloat($('#platform_fees').val()) : 0;
                let bulk_discount = $('#bulk_discount').val() || '0';
                console.log(bulk_discount)
                let final_total = parseFloat(sub_total) + parseFloat(delivery_charge) + parseFloat(platform_fees)
                let wallet_used = $('.wallet_used').text()
                if (wallet_used == '') {
                    wallet_used = 0
                } else {
    ...
    $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (shipping_method == 1) {
                            ...
                            let bulk_discount = $('#bulk_discount').val() || '0';
                            var final_total = parseFloat(sub_total) + parseFloat(delivery_charge_with_cod) + parseFloat(platform_fees) - parseFloat(bulk_discount);
                        } else {
                            var platform_fees = $('#platform_fees').val() ? parseFloat($('#platform_fees').val()) : 0;
                            let bulk_discount = $('#bulk_discount').val() || '0';
                            var final_total = parseFloat(sub_total) + parseFloat(delivery_charge) + parseFloat(platform_fees) - parseFloat(bulk_discount);
                        }
                        $('#amount').val(final_total)
                        final_total = final_total.toLocaleString(undefined, {
                            maximumFractionDigits: 2
        ...
        // Handle platform fees for digital products on page load
        ...
        if (product_type === 'digital_product') {
            ...
            if (platform_fees > 0) {
                var current_final_total = $('#final_total').text().replace(',', '');
                // Check if platform fees are already included
                var expected_total = parseFloat(sub_total) + parseFloat(platform_fees) - parseFloat(bulk_discount);
                var current_total = parseFloat(current_final_total);
                
                if (Math.abs(current_total - expected_total) > 0.01) {
                    // Platform fees not included, add them
                    var final_total = expected_total;
                    $('#amount').val(final_total);
                    final_total = final_total.toLocaleString(undefined, {
                        maximumFractionDigits: 2
                    });
    ...
    $('#pickup_from_store').on('change', function (e) {
        ...
        // Hide address and delivery-related sections
        $('.address').hide()
        $('.charges-section:not(.platform-fees-section)').hide()
        ...
        // Calculate final total with platform fees and bulk discount
        var platform_fees = $('#platform_fees').val() ? parseFloat($('#platform_fees').val()) : 0;
        var bulk_discount = $('#bulk_discount').val() ? parseFloat($('#bulk_discount').val()) : 0;
        ...
        var final_total = parseFloat(sub_total) + parseFloat(platform_fees) - parseFloat(wallet_used) - parseFloat(promocode_amount) - parseFloat(bulk_discount);
        final_total = final_total.toLocaleString(undefined, {
            maximumFractionDigits: 2
        });
    ...
    $('#checkout_form').on('submit', function (event) {
        ...
        if (payment_methods == 'Stripe') {
            ...
            place_order().done(function (result) {
                ...
                $('#csrf_token').val(csrfHash); $.post(base_url + "cart/pre-payment-setup", {
                    [csrfName]: csrfHash,
    ...
    $('#redeem_btn').on('click', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                csrfHash = data.csrfHash;
                if (data.error == false) {
                    let final_discount = parseFloat(data.data[0].final_discount);
                    let is_cashback = data.data[0].is_cashback;
                    let sub_total = $('#sub_total').val().replace(',', '');
                    let wallet_used = $('.wallet_used').text();
                    if (wallet_used == '') {
                        wallet_used = 0;
                    } else {
                    ...
                    let payment_method = $("input[name='payment_method']:checked").val();
                    if (payment_method == "COD") {
                        delivery_charge = $(".delivery_charge_with_cod").val().replace(',', '');
                    } else {
                        delivery_charge = $(".delivery_charge_without_cod").val().replace(',', '');
                    }
                    ...
                    if (is_cashback == 1) {
                        $('#promocode_amount').text(
                            final_discount.toLocaleString(undefined, { maximumFractionDigits: 2 }) +
                            ' (Cashback)'
                        );
                        let final_total = parseFloat(sub_total) + parseFloat(delivery_charge) - parseFloat(wallet_used);
                        $('#final_total').text(
                        ...
                        let final_total = parseFloat(sub_total) + parseFloat(delivery_charge) - parseFloat(wallet_used) - final_discount;
                        $('#final_total').text(
                            final_total.toLocaleString(undefined, { maximumFractionDigits: 2 })
                        );
                        $('#amount').val(final_total);
    ...
    $(document).on('click', '#wallet_balance', function () {
        ...
        let payment_method = $('input[name=payment_method]:checked').val()
        if (payment_method == "COD") {
            var delivery_charge = $(".delivery_charge_with_cod").val().replace(',', '');
        } else {
            var delivery_charge = $(".delivery_charge_without_cod").val().replace(',', '');
        }
        ...
        if ($(this).is(':checked')) {
            ...
            if (final_total - wallet_balance <= 0) {
                ...
                final_total = parseFloat(sub_total) - parseFloat(wallet_balance) - parseFloat(promocode_amount) + parseFloat(delivery_charge) + parseFloat(platform_fees) - parseFloat(bulk_discount);
            ...
            var final_total = parseFloat(sub_total) + parseFloat(delivery_charge) - parseFloat(promocode_amount) + parseFloat(platform_fees) - parseFloat(bulk_discount);
