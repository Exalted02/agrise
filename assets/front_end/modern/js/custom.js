...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (result) {
            ...
            getToken(messaging, { vapidKey: result['vapidKey'] }).then((currentToken) => {
                if (currentToken) {
...
$("#address_list_table").on(
    ...
    function (field, value, row, $el) {
        ...
        $('input[type=radio][value=' + $el.type.toLowerCase() + ']').attr('checked', true);
    });
...
$(document).ready(() => {
    ...
    $('#product-edit-rating-form').on('submit', function (e) {
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function () {
                $('#edit-rating-submit-btn').html('<div class="spinner-border" role="status">' +
                    '<span class="visually-hidden">Loading...</span>' +
                    '</div>').attr('disabled', true);
            },
            success: function (result) {
                ...
                $('#edit-rating-submit-btn').html(submit_btn_html).attr('disabled', false)
            }
        })
    })
                    ...
                    if (compare !== null && compare_count > 0) {
                        ...
                        $.each(result.data.product, function (i, e) {
                            ...
                            comp +=
                                ...
                                '><span class="add-in-cart">Select</span></a>' +
                                '</div></div></a></td>'
                        })
                        comp += '</tr>'
                        comp +=
                        ...
                        comp +=
                            '<tr>' +
                            '<th class="compare-field">Guarantee</th>'
                        $.each(result.data.product, function (i, e) {
                            comp +=
                                '<td class="compare-value" data-title="warranty period">' +
    ...
    $(document).ready(function () {
                    ...
                    if (response.error == false) {
                        ...
                        if (response.data && response.data.is_logged_in == 0) {
                            ...
                            var cart_item = {
                                ...
                                qty: quantity,
                                title: $(this).closest('.product-card').find('.product-title, .card-title').text().trim() || 'Product',
                                description: $(this).closest('.product-card').find('.product-description, .card-text').text().trim() || '',
        ...
        $(document).on('click', '.remove-product', function () {
            ...
            if (typeof is_loggedin !== 'undefined' && is_loggedin == 0) {
                ...
                if (cart !== null) {
                    cart = JSON.parse(cart);
                    cart = cart.filter(item => item.product_variant_id !== productId.toString());
                    localStorage.setItem('cart', JSON.stringify(cart));
                    $('.cart-count').text(cart.length);
                    display_cart(cart);
                    updateCartVisibility();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        csrfName = response['csrfName'];
                        csrfHash = response['csrfHash'];
                        $(`.remove-product[data-id="${productId}"]`).closest('.cart-modal-card').remove();
                        updateCartVisibility();
                    },
                    error: function (xhr, status, error) {
        ...
        $(document).on('change', '.input-field-cart-modal', function () {
            ...
            if (typeof is_loggedin !== 'undefined' && is_loggedin == 0) {
                ...
                if (cart !== null) {
                    cart = JSON.parse(cart);
                    const itemIndex = cart.findIndex(item => item.product_variant_id === productId.toString().trim());
                    if (itemIndex !== -1) {
        ...
        $('.quickview-trigger').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#modal-product-price').text(currency + ' ' + data.variants[0].special_price);
                    ...
                    if (data.type != 'variable_product') {
                        ...
                        $('#buy_now_btn_model').attr('data-product-variant-id', data.variants[0].id);
                    }
            ...
            if ($(this).prop('checked')) {
                ...
                if (selected_attributes.length == attributes_length) {
                    ...
                    if (is_variant_available) {
                        ...
                        $('#buy_it_now').attr('data-product-variant-id', selected_variant_id)
    ...
    $(document).on('click', '.add_to_cart', function (e) {
        e.preventDefault()
        var qty = $(this).parent().parent().parent().find('.input-group-field').val();
        ...
        if (QuickViewModal && QuickViewModal !== '' && $('#modal-product-price').length > 0) {
            ...
            if (modalPriceText && modalPriceText.trim() !== '') {
                ...
                var extractedPrice = modalPriceText.replace(/[^\d.,]/g, '').replace(',', '');
        ...
        if ((!price || price === '' || price === '0') && $('.modal-product-price').length > 0) {
            var hiddenModalPrice = $('.modal-product-price').val();
            if (hiddenModalPrice && hiddenModalPrice.trim() !== '' && !isNaN(parseFloat(hiddenModalPrice))) {
        ...
        if ((!price || price === '' || price === '0') && $('#price').length > 0) {
            ...
            if (displayedPriceText && displayedPriceText.trim() !== '') {
                var pageExtractedPrice = displayedPriceText.replace(/[^\d.,]/g, '').replace(',', '');
                ...
                if (cart !== null && cart.length > 0) {
                    cart.forEach(item => {
                        html +=
                            ...
                            currency +
                            " " +
                            ((item.qty && item.price !== undefined && item.price !== null && !isNaN(item.price)) ? (item.qty * item.price).toLocaleString() : '0') +
                            '</p>' +
                            '</div>' +
                            '</div>' +
                            '</div>' +
    ...
    function display_cart(cart) {
        ...
        console.log(cart.special_price);
        ...
        if (is_loggedin == 0) {
            ...
            var html = ''
            let variant_value = ""
            // var qty = parseFloat(cart.qty) || 1;
            // var price = parseFloat(cart.special_price) || parseFloat(cart.price) || 0;
            // var total = qty * price;
            // console.log(total);

            // if (total > 0) {
            //     final_total = total.toLocaleString();
            // } else {
            //     final_total = "0";
            // }
            if (cart !== null && cart.length > 0) {
                ...
                cart.forEach(item => {
                    ...
                    html +=
                        ...
                        '" step="' +
                        item.step +
                        '"></div><div class="input-group-button"><button type="button" class="button hollow circle plus-btn plus sss" data-quantity="plus" data-field="quantity" data-max="' +
                        item.max +
                        '" data-step="' +
                        item.step +
                        '"><i class="fa-solid fa-plus"></i></button></div></div>' +
            ...
            if (cart !== null && cart.length > 0) {
                ...
                cart.forEach(item => {
                    ...
                    var price = parseFloat(item.special_price) || parseFloat(item.price) || 0;
                    var total = qty * price;
                    var final_total;
    ...
    setTimeout(function () {
        ...
        //         let productQuantity = $(this).closest('.product-quantity');
        ...
        //         let step
        //         if ($(this).hasClass('input-field-cart-modal')) {
        //             id = $(this).data('id');
        //             price = $(this).data('price');
        //             qty = $(this).val();
        //         } else if ($(this).hasClass("plus-btn")) {
        //             let element = $(this).parent().parent().find('.product-quantity').children('.input-field-cart-modal')
        //             id = $(element).data('id');
        ...
        if (filters != '') {
            ...
            $.each(JSON.parse(filters), function (i, e) {
                ...
                $.each(attribute_values, function (j, v) {
                    ...
                    attr_checked_status =
                        $.inArray(decodedValue, selected_attributes) !== -1 ? 'checked' : '';
    ...
    $('.update-order-item').on('click', function (e) {
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                dataType: 'json',
                beforeSend: function () {
                    t.html('<div class="spinner-border" role="status"><span class="visually-hidden">Loading...</span></div>').attr('disabled', true);
                },
                success: function (result) {
                    csrfName = result.csrfName;
                    csrfHash = result.csrfHash;
                ...
                error: function (xhr, status, error) {
                    Toast.fire({
                        icon: 'error',
                        title: 'Server error: ' + (xhr.responseJSON?.message || 'Please try again later.')
                    });
                    t.html(btn_text).attr('disabled', false);
                }
            });
    ...
    $('.update-order').on('click', function (e) {
        console.log("hereljkfjkasjfjlksdajflkjd ");
    ...
    $(document).on('submit', '#send_forgot_password_otp_form', function (e) {
        ...
        if (!captchaResponse) {
            $('#forgot_pass_error_box').html('Please complete the CAPTCHA verification.').show();
            Toast.fire({
                icon: 'error',
    ...
    $('.login-canvas-emailLogin').on('click', function (e) {
        e.preventDefault();
    ...
    $(document).on('click', '.login-section #emailLogin', function (e) {
        e.preventDefault();
    ...
    $(document).on('click', '.login-section #phoneLogin', function (e) {
        e.preventDefault();
    ...
    // Handle phone login button click in modal (only for login modal)
    $('#login-canvas').on('click', '#phoneLogin', function (e) {
        e.preventDefault();
    ...
    $(document).on('click', '.registration-section #emailLogin', function (e) {
        ...
        $('#sign-up-form').append('<input type="hidden" name="type" value="email">');
    });
    ...
    $(document).on('submit', '#sign-up-form', function (e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.error == true) {                    // Create the user in Firebase
                    firebase.auth().createUserWithEmailAndPassword(email, password)
                        .then(function (userCredential) {
                            ...
                            $.ajax({
                                ...
                                success: function (result) {
                                    ...
                                    Toast.fire({
                                        icon: 'success',
                                        title: 'Registration successful! Please verify your email before logging in.'
                                    });
                                    ...
                                    if (result.error == false) {
                                        ...
                                        $('#sign-up-error').html('<div class="alert alert-danger">' + result.message + '</div>');
                                    }
                                }
                            });
                        })
                        .catch(function (error) {
                            ...
                            $('#sign-up-error').html('<div class="alert alert-danger">' + errorMessage + '</div>');
                        });
                } else {
                    $('#sign-up-error').html('<div class="alert alert-danger">' + 'Email is already registered' + '</div>');
                }
            }
        });
    });
    ...
    $(document).on('submit', '#create_ticket_form', function (event) {
        ...
        var type = $('#ticket_type_select').val();
        var email = $('#email').val();
...
$(document).on('submit', '.sign-up-form', function (e) {
    ...
    var isPhoneRegistration = $('#phone-number').length > 0 && $('#phone-number').is(':visible');
...
// $('#chat_user').select2({
//     ajax: {
//         url: base_url + "/admin/Chat/get_members",
//         data: function (params) {
//             var query = {
//                 search: params.term,
//             }
...
$(document).on('click', '.confirmReturn', function (e) {
    e.preventDefault();
    console.log('Confirm Return clicked for item ID:', $(this).data('item-id'));
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            ...
            Toast.fire({
                icon: 'error',
                title: 'Server error: ' + (xhr.responseJSON?.message || 'Please try again.')
            });
            $button.prop('disabled', false).text('Confirm Return');
        }
    });
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.error == false) {
                        ...
                        $('#send_bank_receipt_form').hide();
                        $('#send_bank_receipt_form').after(
                            '<div class="alert alert-success">' +
                            (!empty($this.lang.line('receipt_uploaded')) ? $this.lang.line('receipt_uploaded') : 'Bank payment receipt uploaded successfully.') +
                            '</div>'
                        );
                        $('table').bootstrapTable('refresh');
                        Toast.fire({
                            icon: 'success',
                            title: result.message || 'Bank payment receipt uploaded successfully.'
                        });
                    } else {
                        // Handle server errors (e.g., receipt already uploaded or invalid file)
                        Toast.fire({
                            icon: 'error',
...
$(document).ready(() => {
    ...
    $('#forgot-password-email-form').on('submit', function (e) {
        ...
        if (!email) {
            $('#forgot_pass_email_error_box').html('<span class="text-danger">Please enter your email.</span>');
            return;
        }
        $('#send_firebase_reset_email_btn').attr('disabled', true).text('Sending...');
        firebase.auth().sendPasswordResetEmail(email)
            .then(function () {
                $('#forgot_pass_email_error_box').html('<span class="text-success">Password reset email sent! Please check your inbox.</span>');
                $('#send_firebase_reset_email_btn').attr('disabled', false).text('Send Reset Email');
            })
            .catch(function (error) {
                $('#forgot_pass_email_error_box').html('<span class="text-danger">' + error.message + '</span>');
                $('#send_firebase_reset_email_btn').attr('disabled', false).text('Send Reset Email');
            });
    });
                ...
                firebase.auth().confirmPasswordReset(oobCode, newPassword)
                    .then(() => {
                        ...
                        $.ajax({
                            ...
                            beforeSend: function () {
                                console.log("Sending request to reset MySQL password..." + email + " " + newPassword);
                            },
                            success: function (response) {
                                ...
                                if (res.error) {
                                    ...
                                    Toast.fire({
                                        icon: "success",
                                        title: "Password has been reset successfully in both systems!"
                                    });
                                    resetForm.style.display = "none";
                                    location.href = base_url;
                                }
    ...
    $('.quickview-variant-sec').each(function () {
        ...
        if ($swatches.length === 1) {
            ...
            if ($radioInput.length > 0 && !$radioInput.prop('checked')) {
                ...
                $swatches.addClass('border-active').siblings().removeClass('border-active');
    ...
    $(document).on('shown.bs.modal', '#quickview', function () {
        $('.quickview-variant-sec').each(function () {
            ...
            if ($swatches.length === 1) {
                var $radioInput = $swatches.find('input[type="radio"].modal-product-attributes');
                if ($radioInput.length > 0 && !$radioInput.prop('checked')) {
                    $radioInput.prop('checked', true);
    ...
    $(document).on('click', '.num-block .plus-btn, .num-block .minus-btn', function (e) {
        var input = $(this).parents('.num-block').find('input');
        ...
        if (input.data('page') === 'cart' && is_loggedin == 1) {
            ...
            if (new_qty !== current_qty) {
                ...
                if (typeof csrfName !== 'undefined' && typeof csrfHash !== 'undefined' && csrfName && csrfHash) {
    ...
    $(document).on('change', '.input-field-cart-modal', function () {
        ...
        // if (input.data('page') === 'cart' && is_loggedin == 1) {
        console.log('Cart input change detected');
        var product_variant_id = input.data('id');
    ...
    $(document).on('submit', 'form[action*="login/update_user"]', function (e) {
        ...
        if (oldPassword && newPassword && confirmPassword) {
            ...
            if (userType === 'email' && typeof firebase !== 'undefined' && firebase.auth) {
                var user = firebase.auth().currentUser;

                if (user && user.email === userEmail) {
                    // Re-authenticate user with old password first
                    var credential = firebase.auth.EmailAuthProvider.credential(userEmail, oldPassword);
                    ...
                    user.reauthenticateWithCredential(credential)
                        ...
                        .then(function (response) {
                            if (response.error === false) {
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        if (result.error === false) {
                                            ...
                                            Toast.fire({
                                                icon: 'warning',
                                                title: 'Password updated but profile update failed: ' + (result.message || 'Unknown error')
                                            });
                                        }
                                    },
                                    error: function () {
                                ...
                                Toast.fire({
                                    icon: 'error',
                                    title: response.message || 'Failed to update password in database'
                                });
                            }
                        })
                        .catch(function (error) {
